...
// Client Message: SEQ_ID(2 bytes) + MSG_TYPE(1 byte) + ERROR_CODE(1 byte) + DATA
class ClientMessage {
public:
    static constexpr size_t HEADER_SIZE = 4;  // Maximum IPC packet length

    uint16_t seq_;              // SEQ_ID (2 bytes, little endian)
    SpiCommon::MsgType  msg_t_;   // MSG_TYPE (1 byte)
    ...
    std::string toString() const {
        ...
        oss << "SEQ=0x" << std::hex << std::setw(4) << std::setfill('0') << seq_
            << ", TYP=0x" << std::hex << std::setw(2) << std::setfill('0') << static_cast<int>(msg_t_)
